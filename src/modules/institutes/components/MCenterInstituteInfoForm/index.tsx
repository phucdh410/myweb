...
import { IMCenterInstituteInfoFormProps, TabPanelProps } from './types';
...
export const MCenterInstituteInfoForm: React.FC<
  IMCenterInstituteInfoFormProps
> = ({ department_id, data }) => {
  //#region Data
      ...
      <div style={{ marginTop: 40 }}>
        <TabPanel value={0} current={currentTab}>
          <MIntroduceForm
            ...
            data={data?.centers_institutes_introduces}
          />
        </TabPanel>
        <TabPanel value={1} current={currentTab}>
          <MTimelineForm
            department_id={department_id}
            data={data?.centers_institutes_timelines}
          />
        </TabPanel>
        <TabPanel value={2} current={currentTab}>
          <MMissionForm
            department_id={department_id}
            data={data?.centers_institutes_function_missions}
          />
        </TabPanel>
        <TabPanel value={3} current={currentTab}>
          <MOrganizationalStructureForm
            department_id={department_id}
            data={data?.centers_institutes_organizational_structures}
          />
        </TabPanel>
        <TabPanel value={4} current={currentTab}>
          <MActiveForm
            department_id={department_id}
            data={data?.centers_institutes_actives}
          />
        </TabPanel>
        <TabPanel value={5} current={currentTab}>
          <MLeadershipForm
            department_id={department_id}
            data={data?.centers_institutes_leadership_through_the_ages}
          />
        </TabPanel>
        <TabPanel value={6} current={currentTab}>
          <MContactForm
            department_id={department_id}
            data={data?.centers_institutes_contacts}
          />
        </TabPanel>
      </div>
    </Stack>
