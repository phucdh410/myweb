...
const CreateFunctionPage = () => {
  ...
  const onSubmit = () => {
    handleSubmit(async (values) => {
      try {
        ...
        toast.success(toastMessage('phòng ban').SUCCESS.ADD);
        onCancel();
      } catch (error: any) {
        toast.error(
          error?.response?.data?.message || toastMessage('phòng ban').ERROR.ADD,
        );
      }
    })();
  };
  ...
  return (
    <>
      <Box mb={3}>
        <Typography variant="page-title">Thêm mới phòng ban</Typography>
      </Box>
